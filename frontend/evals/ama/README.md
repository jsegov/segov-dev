...
The runtime removes successfully executed retrieval tools from the SDK's available and executable tool set each step, and enables resume retrieval only after public retrieval. A per-step system reminder lists only known tool names, preserves earlier results as usable context, and points to the resume when a public background fact is missing. The prompt manifest records `single-use-context-v3`. Rejected calls may be corrected within the existing step limit; they remain visible in traces and evaluation scoring. The current training builders require fixed tool declarations throughout a turn, so traces carrying this dynamic policy are excluded and fail preflight even if approved. See [training provenance rules](../../../training/README.md). Existing static-policy snapshots remain reproducible.
