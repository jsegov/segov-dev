...
Paths supplied to the package script resolve from `training/`. Build performs no network work. It reports separate exclusion counts for pending, rejected, changed, superseded, wrong-model, wrong-prompt, unsupported tool-availability policy, unfinished, visitor, and evaluation-overlapping rows. Changed approval hashes exclude those rows; a changed sealed snapshot still aborts the build. Regeneration resolution runs against the complete snapshot before eligibility filtering. Qwen uses approved per-turn histories. Inkling requires contiguous, identical-prefix histories with one prompt version and approval for every selected turn; pruned histories remain per-turn only.

Training currently supports legacy fixed tool declarations and system instructions only, identified by an absent `toolAvailabilityPolicy`. Any present policy, including `single-use-context-v3` or an unknown value, excludes the trace even if approved. The derived prompt manifest separates this metadata from the persisted `call_settings` envelope; the original snapshot stays unchanged. Python preflight checks both source and derived prompt metadata for every included trace. Mixed snapshots can retain excluded dynamic prompt versions, but cannot train on them. A build with no eligible examples fails with exclusion counts and creates no dataset. Current dynamic traces require future step-specific inputs, system instructions, active tool declarations and assistant targets before they can preserve training/serving fidelity. The existing per-turn construction and template-parity checks cannot reproduce changing tool availability or per-step system reminders; retaining earlier tool results as usable context does not restore that missing prompt fidelity.
...
Inspect `data/checkpoints.json` for candidate IDs. Use the exact original training overrides for `score` and `sample`, including the original warm-start setting; changed code/data/dependencies/configuration is rejected. These evidence commands recover the original parent checkpoint from the sealed candidate, so an advanced `latest_training_state` pointer cannot change their configuration. Validation and resumed training in the same `log_path` recover that parent from its sealed run/preflight manifests while the cookbook resumes the latest checkpoint within the run. Explicit conflicting checkpoint overrides fail; starting a new run requires a new `log_path`. Both evidence commands accept `registry=PATH` for a separate registry.
...
This command claims a unique final attempt in the registry **before any model calls**, invokes the frozen production frontend suite and locked-winner final NLL, verifies both attempt-bound reports, and automatically promotes if they pass. It supplies the sealed checkpoint decision so the frontend runner can verify winner bindings and evaluation configuration before final model calls. It supplies the exact verified endpoint, artifact alias and inference settings. Existing endpoint/judge credentials are used but never written to evidence. A failure or interruption consumes the attempt; even an incomplete attempt cannot be retried or replaced by a runner-up. Output directories must be new. The lower-level `promote --decision ... --final ... --final-nll ...` only accepts reports from an already claimed attempt.
